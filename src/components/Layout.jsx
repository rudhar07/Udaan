...
function Layout() {
  ...
  return (
    <div className={`min-h-screen ${isDarkMode ? 'bg-slate-900 text-white' : 'bg-gradient-to-b from-slate-50 to-slate-100'}`}>
      {/* Header */}
      <header className={`${isDarkMode ? 'bg-gradient-to-r from-slate-800 to-slate-900' : 'bg-gradient-to-r from-blue-600 to-indigo-700'} text-white py-5 px-6 shadow-lg`}>
          ...
          <div className="flex items-center space-x-4">
            ...
            {currentUser ? (
              <div className="relative" ref={dropdownRef}>
                ...
                {dropdownOpen && (
                  <div className={`absolute right-0 mt-2 w-48 ${isDarkMode ? 'bg-slate-800 text-white' : 'bg-white text-slate-900'} rounded-lg shadow-lg py-1 z-10`}>
                    <div className={`px-4 py-3 border-b ${isDarkMode ? 'border-slate-700' : 'border-gray-100'}`}>
                      <p className={`text-sm font-medium ${isDarkMode ? 'text-white' : 'text-gray-900'} truncate`}>{currentUser.displayName}</p>
                      <p className="text-xs text-gray-500 truncate">{currentUser.email}</p>
                    </div>
                    <button 
                      onClick={handleSignOut}
                      className={`block w-full text-left px-4 py-2 text-sm ${isDarkMode ? 'text-gray-300 hover:bg-slate-700' : 'text-gray-700 hover:bg-gray-100'}`}
                    >
                      Sign out
                    </button>
                  </div>
      ...
      <main className="max-w-7xl mx-auto p-4 relative -mt-16">
        <div className={`${isDarkMode ? 'bg-slate-800' : 'bg-white'} rounded-xl shadow-xl p-6`}>
          <Outlet />
        </div>
      </main>
      
      {/* Footer */}
      <footer className={`${isDarkMode ? 'bg-slate-900' : 'bg-gradient-to-r from-slate-800 to-slate-900'} text-white mt-12 py-10 px-6`}>
